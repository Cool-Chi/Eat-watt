...
🎉 **目前版本：v1.4.1 博韋 (Beauvais)** *Beauvais 更新帶來了「多維度精準篩選系統」與「物理級別的折疊展開動畫」，大幅提升了行動端的垂直空間利用率與過濾精準度！*
...
* **多維度精準篩選 (v1.4 New!)**：
  * 支援 $（平價）、$$（中價位）、$$$（奢華）預算多選篩選。
  * **資料夾膠囊過濾**：自動提取頂層資料夾生成 Emoji 分類膠囊，支援與預算進行「交集 (AND) 過濾」，精準收斂晚餐名單。
  * **折疊式預覽名單**：採用 CSS Grid 高度坍塌技術實作的絲滑展開動畫，不再佔用過多畫面空間，點擊即可彈性推擠預覽結果。
* **資料夾專屬圖標選單**：點擊資料夾圖示可直接喚醒具備毛玻璃質感的全域懸浮 Emoji 選擇器，快速切換美食分類標籤。
...
├── sw.js               # Service Worker 快取腳本 (v1.4)
├── css/                # 模組化樣式
│   ├── base.css        # 全域變數、深淺模式切換與全域過渡
│   ├── layout.css      # 版面佈局與 Grid 網格結構
│   ├── components.css  # 主題開關、過濾膠囊、折疊動畫與 Modal
